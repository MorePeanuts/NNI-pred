...
class FeaturePreprocessor:
    ...
    def separate_features_targets(self) -> Tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame]:
        ...
        target_cols = ['THIA', 'IMI', 'CLO', 'ACE', 'DIN', 'parentNNIs',
        ...
        self.metadata = self.df[metadata_cols].copy()
    ...
    def diagnose_skewness(self, X: pd.DataFrame) -> Dict[str, float]:
        ...
        high_skew_count = sum(1 for v in skewness_dict.values()
                            if abs(v) > self.skewness_threshold)
    ...
    def define_feature_groups(self) -> Dict[str, List[str]]:
        ...
        rfp_pollutants = ['THIA', 'IMI', 'CLO', 'parentNNIs', 'IMI-UREA',
                         'DN-IMI', 'CLO-UREA', 'mNNIs']
        ...
        # Physicochemical (9)
        group1_natural.extend(['DO', 'pH', 'COND', 'DOC',
                             'Soil_pH_wMean', 'Soil_TN_wMean', 'Soil_TOC_wMean',
                             'Soil_CC_wMean', 'Soil_BD_wMean'])

        # Vegetation & Topography (7 - including LST)
        group1_natural.extend(['Alt', 'Soil_Alt_wMean', 'Soil_NDVI_wMean',
                             'Soil_EVI_wMean', 'Soil_LAI_wMean', 'Soil_FCOVER_wMean',
                             'Soil_LST_wMean'])
        ...
        group2_agro = [
            # Inputs (10)
            'FER', 'Soil_FER_wMean', 'FERPER', 'Soil_FERA_wMean',
            'PES', 'Soil_PES_wMean', 'PESPER', 'Soil_PESA_wMean',
            'AMP', 'Soil_GPAM_wMean',
            # Crop Area (7)
            'TSA', 'FCA', 'WA', 'Soil_WS_wMean', 'CA', 'VEGA', 'ARCA',
            # Yields (13)
            'CROPOUT', 'WO', 'Soil_MC_wMean', 'CO', 'VO', 'Soil_VE_wMean',
            'FOP', 'Soil_FR_wMean', 'Soil_MCP_wMean', 'Soil_VEP_wMean', 'Soil_FRP_wMean',
            # Water (4) - but CROPOUT is missing one, need to add IRR_W
        ]
        # Add remaining water variables
        ...
        group3_socio = [
            # Macro Economy (8)
            'GDP', 'Soil_GDP per capita_wMean', 'OP_FI', 'OP_SE', 'OP_TH',
            'AO', 'FO', 'Soil_GAO_wMean',
            # Urbanization (3)
            'Urban', 'Soil_UR_wMean', 'POP_TOT',
            # Structure & Income (10)
            'PR', 'Soil_PR_wMean', 'SR', 'Soil_SR_wMean', 'TR', 'Soil_TR_wMean',
            'UI', 'Soil_UI_wMean', 'RI', 'Soil_RI_wMean',
            # Non-Ag Water (8)
            'UR_W', 'Soil_UR_W_wMean', 'RU_W', 'Soil_RU_W_wMean',
            'IND_W', 'Soil_IND_W_wMean', 'LIF_W', 'Soil_LIF_W_wMean'
        ]
    ...
    def encode_categorical(self, X: pd.DataFrame, cat_cols: List[str]) -> Tuple[pd.DataFrame, OneHotEncoder]:
        """
    ...
    def process_group1_natural(self, X_group1: pd.DataFrame) -> pd.DataFrame:
        ...
        X_scaled = pd.DataFrame(
            X_scaled,
            columns=X_group1.columns,
            index=X_group1.index
        )
    ...
    def process_group2_agro_pca(self, X_group2: pd.DataFrame) -> Tuple[pd.DataFrame, PCA, StandardScaler]:
        """
    ...
    def process_group3_socio_pca(self, X_group3: pd.DataFrame) -> Tuple[pd.DataFrame, PCA, StandardScaler]:
        """
    ...
    def combine_all_features(self,
                           ...
                           X_group3_pca: pd.DataFrame) -> pd.DataFrame:
        """
    ...
    def save_transformers(self, output_dir: str):
        ...
        print(f"✓ Saved: feature_groups.pkl")
    ...
    def generate_report(self, output_dir: str):
        ...
        with open(report_file, 'w', encoding='utf-8') as f:
            ...
            transform_info = self.skewness_transformer['transformation_info']
            transformed_count = sum(1 for v in transform_info.values()
                                  if v['method'] != 'unchanged')
            ...
            sorted_skew = sorted(self.skewness_original.items(),
            ...
            for feat, _ in sorted_skew[:10]:
                ...
                f.write(f"{feat:<35} {info['original_skew']:>15.4f} {info['new_skew']:>15.4f} {info['method']:<20}\n")
            f.write("\n")
            ...
            f.write(f"Group 1 (Natural & Spatial): {len(self.feature_groups['group1_natural'])} features\n")
            f.write(f"  - Processing: Standardization only (no PCA)\n\n")

            f.write(f"Group 2 (Agricultural Intensity): {len(self.feature_groups['group2_agro'])} features\n")
            f.write(f"  - Processing: Standardization + PCA (95% variance)\n")
            f.write(f"  - Output: {self.group2_pca.n_components_} principal components\n\n")

            f.write(f"Group 3 (Socio-Economic): {len(self.feature_groups['group3_socio'])} features\n")
            f.write(f"  - Processing: Standardization + PCA (95% variance)\n")
            ...
            f.write(f"  - Processing: One-hot encoding (drop first)\n")
            f.write(f"  - Output: {len([c for c in self.X_final.columns if any(cat in c for cat in self.feature_groups['categorical'])])} features\n\n")

            # 3. PCA Details - Group 2
            f.write("3. PCA ANALYSIS - GROUP 2 (AGRICULTURAL INTENSITY)\n")
            f.write("-" * 80 + "\n\n")

            explained_var2 = self.group2_pca.explained_variance_ratio_
            ...
            f.write("-" * 45 + "\n")
            for i, (var, cumvar) in enumerate(zip(explained_var2, cumsum_var2)):
            ...
            components2 = self.group2_pca.components_
            ...
            explained_var3 = self.group3_pca.explained_variance_ratio_
            ...
            f.write(f"{'Component':<15} {'Variance':>15} {'Cumulative':>15}\n")
            f.write("-" * 45 + "\n")
            for i, (var, cumvar) in enumerate(zip(explained_var3, cumsum_var3)):
            ...
            components3 = self.group3_pca.components_
            ...
            group1_cols = [c for c in self.X_final.columns if c in self.feature_groups['group1_natural']]
            cat_cols = [c for c in self.X_final.columns if any(cat in c for cat in self.feature_groups['categorical'])]
            agro_cols = [c for c in self.X_final.columns if c.startswith('PC_Agro_')]
            socio_cols = [c for c in self.X_final.columns if c.startswith('PC_Socio_')]
    ...
    def _generate_visualizations(self, output_path: Path):
        ...
        axes[0].barh(range(len(skew_original)), skew_original.values)
        axes[0].set_yticks(range(len(skew_original)))
        axes[0].set_yticklabels(skew_original.index, fontsize=8)
        axes[0].axvline(x=self.skewness_threshold, color='r', linestyle='--', label=f'Threshold: {self.skewness_threshold}')
        ...
        skew_transformed = pd.Series({k: self.skewness_transformed[k] for k in skew_original.index})
        ...
        axes[1].axvline(x=self.skewness_threshold, color='r', linestyle='--', label=f'Threshold: {self.skewness_threshold}')
...
def main():
    ...
    X_group1_processed = preprocessor.process_group1_natural(X_group1)
    X_group2_pca, pca2, scaler2 = preprocessor.process_group2_agro_pca(X_group2)
    X_group3_pca, pca3, scaler3 = preprocessor.process_group3_socio_pca(X_group3)
