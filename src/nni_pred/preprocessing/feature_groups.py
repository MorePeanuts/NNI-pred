"""
Feature Group Definitions for NNI Prediction

This module defines the feature groups based on domain knowledge from the research plan.
Features are divided into three groups plus categorical variables:
- Group 1: Natural & Spatial Heterogeneity (no PCA, standardization only)
- Group 2: Agricultural Intensity (standardization + PCA)
- Group 3: Socio-Economic Development (standardization + PCA)
- Categorical: Season, Landuse, Soil_landuse (one-hot encoding)

Note: This function expects data from idw_merge.py which uses IDW aggregation,
NOT from sfe_merge.py which uses RFP method.
"""
...
def validate_feature_groups(df_columns: list) -> dict:
    """
    ...
    Returns:
        ...
        - 'missing': Features defined but not in dataframe
        - 'extra': Features in dataframe but not defined
        - 'valid': Boolean indicating if validation passed
    """
    feature_groups = get_feature_groups()

    # All defined features (excluding metadata and targets)
    all_defined = (
        feature_groups['group1_natural'] +
        feature_groups['group2_agro'] +
        feature_groups['group3_socio'] +
    ...
    # Exclude metadata and targets from column check
    exclude_cols = set(feature_groups['metadata'] + feature_groups['targets'])
    df_features = set(df_columns) - exclude_cols

    # Find discrepancies
    missing = defined_set - df_features
    extra = df_features - defined_set
